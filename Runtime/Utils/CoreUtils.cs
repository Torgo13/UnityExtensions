    ...
    public static class CoreUtils
    {
        ...
        public struct CalculateViewSpaceCornersJob : Unity.Jobs.IJob
        {
            ...
            public void Execute()
            {
                ...
                temp[0] = invProj.MultiplyPoint(new Vector3(-1, -1, projZ));
                temp[1] = invProj.MultiplyPoint(new Vector3(1, -1, projZ));
                temp[2] = invProj.MultiplyPoint(new Vector3(1, 1, projZ));
                temp[3] = invProj.MultiplyPoint(new Vector3(-1, 1, projZ));
        ...
        public static void CopyTo<T>(this List<T> list, Span<byte> dest, int count)
            where T : struct
        {
            CopyTo(list.AsSpan(start: 0, count), dest);
        }
        ...
        public static void CopyTo<T>(this Span<T> list, Span<byte> dest, int count)
            where T : struct
        {
            CopyTo(list.Slice(start: 0, count), dest);
        }

        public static void CopyTo<T>(this Span<T> list, Span<byte> dest)
            where T : struct
        {
        ...
        public static void MergeSort(Span<uint> arr, int sortSize, ref Span<uint> supportArray)
        {
            ...
            sortSize = System.Math.Min(sortSize, arr.Length);
            if (sortSize == 0)
                return;
        ...
        public static void InsertionSort(uint[]? arr, int sortSize)
        {
            ...
            sortSize = System.Math.Min(arr.Length, sortSize);
            if (sortSize == 0)
                return;
        ...
        {
            sortSize = System.Math.Min(sortSize, array.Length);
            CalculateRadixParams(radixBits, out int bitStates);
